...
class Comment
{
	...
	public function markup()
	{
		...
		*/
		...
		if($d['url']){
			...
			$link_open = '<a class="comment-card__link" href="'.$d['url'].'">';
			$link_close =  '</a>';
		}
		...
		return '
			<div class="comment-card">
				<div class="comment-card__contain">
					...
					<div class="comment-card__text">
	                    <div class="comment-card__wrap">
							<div class="comment-card__name">'.$link_open.$d['name'].$link_close.'</div>
							<div class="comment-card__date" title="Added at '.date('H:i \o\n d M Y',$d['dt']).'">'.date('d M Y',$d['dt']).'</div>
						</div>
						<p>'.$d['body'].'</p>
					</div>	
				</div>
	...
	public static function validate(&$arr)
	{
		...
		/	массив $arr, который передается как параметр либо данными либо сообщением об ошибке.
		*/
